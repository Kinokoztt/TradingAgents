...
| `CUDA out of memory` at load | Model too big for 48 GB at this quant/context | Use an INT4/AWQ build; lower `MAX_MODEL_LEN`; keep `KV_CACHE_DTYPE=fp8_e5m2`; reduce `GPU_MEM_UTIL`. |
| `ValueError: type fp8e4nv not supported in this architecture ... ('fp8e4b15', 'fp8e5')` (Triton autotuning at startup) | `--kv-cache-dtype fp8` means e4m3, which is Hopper/Ada-only; 3090s (Ampere) only do e5m2 | Use `KV_CACHE_DTYPE=fp8_e5m2` (default in serve_vllm.sh) or `auto` to disable fp8 KV cache. |
