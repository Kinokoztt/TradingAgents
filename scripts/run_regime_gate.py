...
def main() -> int:
    ...
    if as_of == "latest":
        ...
        as_of = _dt.datetime.now(zoneinfo.ZoneInfo("America/New_York")).strftime("%Y-%m-%d")
    ...
    print(f"stocks={len(report.stock_signals)}  long={len(report.long_whitelist)}  "
          f"short={len(report.short_whitelist)}  block={len(report.block_list)}")
    ...
    print(f"long whitelist: {report.long_whitelist}")
